...
import execute.exu.{StdExu, StdMouExu}
import freechips.rocketchip.diplomacy.LazyModule

class StdMouComplex(id: Int)(implicit p:Parameters) extends BasicExuComplex{
  val bypassNum = 0
  val std = LazyModule(new StdMouExu(id,"StdMouComplex", bypassNum))
