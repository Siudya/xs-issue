...
abstract class BasicExuImpl(outer:BasicExu) extends LazyModuleImp(outer){
  ...
  def bypassSigGen(bypassIn:Seq[Valid[ExuOutput]], issuePort:IssueBundle, hasBypass:Boolean):Valid[ExuInput] = {
    ...
    finalIssueSignals.bits.uop := issuePort.issue.uop
    finalIssueSignals.bits.src := issuePort.issue.src
    if(hasBypass) {
      val bypass = bypassIn
      val bypassData = bypass.map(_.bits.data)
      val bypassSrc0Hits = bypass.map(elm => elm.valid && elm.bits.uop.pdest === issuePort.issue.uop.psrc(0))
      val bypassSrc1Hits = bypass.map(elm => elm.valid && elm.bits.uop.pdest === issuePort.issue.uop.psrc(1))
      ...
      finalIssueSignals.bits.src(0) := Mux(bypassSrc0Valid, bypassSrc0Data, issuePort.issue.src(0))
      finalIssueSignals.bits.src(1) := Mux(bypassSrc1Valid, bypassSrc1Data, issuePort.issue.src(1))
    }
    finalIssueSignals
  }
}
